...
from schemas.ResourceImages import ResourceImages as ResourceImageSchema, ResourceImageCreate as ResourceImageCreateSchema
...
def get_resource_images(db: Session = Depends(get_db)):
    try:
        resource_images = db.query(ResourceImage).all()
...
def get_single_resource_image(resource_image_id: int, db: Session = Depends(get_db)):
    try:
        resource_image = db.query(ResourceImage).filter(ResourceImage.id == resource_image_id).first()
...
def create_new_resource_image(resource_image: ResourceImageCreateSchema, db: Session = Depends(get_db)):
    try:
        resource = db.query(Resources).filter(Resources.id == resource_image.resource_id).first()
        if resource is None:
            raise HTTPException(status_code=404, detail="Resource not found")
        
        db_resource_image = ResourceImage(resource_id=resource_image.resource_id, image_path=resource_image.image_path, order=resource_image.order)
...
def update_resource_image(resource_image_id: int, resource_image: ResourceImageCreateSchema, db: Session = Depends(get_db)):
    try:
        db_resource_image = db.query(ResourceImage).filter(ResourceImage.id == resource_image_id).first()
